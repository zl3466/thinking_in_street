...
from train.data_loader.waymo import WaymoDataset
import cv2
...
video_length_list = [2, 4, 6, 8, 10, 16, 24, 32]
    ...
    for frame_i in range(len(ego_pose_list)):
        ...
        if sample_count != 0 and sample_count % (batch_size - 1) == 0:
            ...
            if has_invalid_values(batch_disp_gt) or has_invalid_values(batch_delta_heading_gt) or has_invalid_values(
                ...
                print(f"Skipping invalid example in {dataset_name}: scene_{scene_idx}, frame {frame_i}/{len(ego_pose_list)}")
