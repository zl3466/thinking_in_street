...
area_corner_list = [[40.689981, -73.983547], [40.691480, -73.987365], [40.694969, -73.987166], [40.694847, -73.984001],
                    [40.689973, -73.981591]]

# Base directories
BASE_DATA_DIR = "C:/Users/ROG_ZL/Documents/github/Thinking_in_Street/data/long_grid"
BASE_VIDEO_DIR = "C:/Users/ROG_ZL/Documents/github/Thinking_in_Street/videos/long_grid"


def area2grid(lat_lng_corners, grid_size):
    lat = lat_lng_corners[:, 0]
    lng = lat_lng_corners[:, 1]
    min_lat, max_lat = min(lat), max(lat)
    min_lon, max_lon = min(lng), max(lng)

    # Approximate meters to degrees conversion (varies with latitude)
    lat_step = geodesic(meters=grid_size).destination((min_lat, min_lon), 0)[0] - min_lat
    lon_step = geodesic(meters=grid_size).destination((min_lat, min_lon), 90)[1] - min_lon
...
def fetch_street_view_grid(lat, lng, api_key, heading, size="640x400", pitch=0, fov=120, folder="frames",
                           seen_hashes=None, frame_index=0):
    if seen_hashes is None:
        seen_hashes = set()
    folder = f"{folder}/{heading}"
...
def fetch_street_view_grid_cat(lat, lng, api_key, heading_list, size="640x400", pitch=0, fov=120, folder="frames",
                           seen_hashes=None, frame_index=0):
    if seen_hashes is None:
        seen_hashes = set()
    folder = f"{folder}/cat"
    ...
    # The leading zeros in index ensure correct alphabetical sorting if needed
    filename = (f"{folder}/frame_{frame_index:05d}_"
                f"{lat:.5f}_{lng:.5f}_360.jpg")

    cv2.imwrite(filename, surround_view_img)
    # with open(filename, 'wb') as f:
    #     f.write(surround_view_img)
    return filename
...
#     location_folder = get_location_folder_name(START_LOCATION)
...
#     overview_poly = get_directions(START_LOCATION, END_LOCATION, GOOGLE_MAPS_API_KEY)
#
#     # Create map visualization (optional)
#     create_map_visualization(overview_poly)
#
#     # 2. Decode polyline
...
#
#     for idx, (lat, lng, heading, movement) in enumerate(all_positions_and_headings):
...
def main():
    map_name = "map_2_downtownbk_test"
    # Create base directories if they don't exist
    ...
    # Initialize data storage
    route_data = {
        "area_corner_coords": area_corner_list,
        "grid_size": grid_size,
        "frames": []
    }
    ...
    grid_coord_list = area2grid(area_corner_arr, grid_size)
    frame_files = []
    seen_hashes = set()
    total_frames = len(grid_coord_list)

    valid_coord_list = []
    for idx, (lat, lng) in tqdm(enumerate(grid_coord_list), total=len(grid_coord_list)):
        reverse_idx = total_frames - 1 - idx
        ...
        fname = fetch_street_view_grid_cat(
            lat, lng,
            api_key=GOOGLE_MAPS_API_KEY,
            seen_hashes=seen_hashes,
            folder=frames_dir,
        ...
        if fname:
            ...
            valid_coord_list.append([lat, lng])

    # Initialize a map centered at the first coordinate
    m = folium.Map(location=valid_coord_list[0], zoom_start=5, tiles="OpenStreetMap")

    # Add each coordinate as a marker
    for [lat, lng] in valid_coord_list:
    ...
    coord_file = os.path.join(data_dir, "coord_data.json")
    with open(coord_file, 'w') as f:
        json.dump(valid_coord_list, f, indent=2)
    # Generate video
    # if frame_files:
    #     video_file = os.path.join(BASE_VIDEO_DIR, f"{location_folder}.mp4")
    #     success = generate_video_from_img(frames_dir, video_file, frame_rate=5, reverse=False)
