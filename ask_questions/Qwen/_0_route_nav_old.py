...
def main():
    ...
    result_dir = "../../eval_result/route_nav"
    ...
    for map_name in map_list:
        ...
        for i in range(len(task_list)):
            ...
            ans_choice_dict = {"direction_walking": {"gt": correct_idx_walking, "choices": final_choices_walking},
            ...
            question_list = [
                f"\nThese are frames of a video. They cover all streets within an area. \n"
                f"I want to walk through the locations: {formatted_locations} in this exact order, with the "
                f"starting position being facing {question_location_list[0]}. \n"
                ...
                # f"\nThese are frames of a video. They cover most of the streets within an area. \n"
                # f"I want to drive through the locations: {formatted_locations} in this exact order, with the "
                # f"starting position being facing {question_location_list[0]}. Since you are driving, you should "
                # f"consider constraints such as a street being one-way drive.\n"
                # f"Put your single letter choice answer between the <answer> </answer> tags tags\n"
                ...
                f"\nThese are frames of a video. They cover all streets within an area. \n"
                f"I want to walk through the locations: {formatted_locations} in this exact order, "
                ...
                f"\nThese are frames of a video. They cover all streets within an area. \n"
                f"Can you choose the optimal order of visiting all these locations that gives the shortest path : "
                f"{formatted_locations_multi}?\n"
                f"Put your single letter choice answer between the <answer> </answer> tags tags\n"
            ...
            # video_img_paths = os.listdir(f"{map_name}/frames")
            ...
            if results:
                ...
                for question, answer in results.items():
                    ...
                    with open(f"{set_out_dir}/Q&A.md", "a", encoding="utf-8") as file:
                        ...
                        file.write(f"\n----------\nTrue Correct Answer: "
                                   f"{idx2letter[[correct_idx_walking, correct_idx_distance_walking, correct_idx_optm][idx]]}\n==========\n")
            ...
            final_choices_optm_gt = [location_dict[key] for key in final_choices_optm[correct_idx_optm]]

            all_positions_and_headings, _, _ = get_routes_from_locations(final_choices_optm_gt,
                                                                         f"{set_out_dir}/route_map_multi_stop_optimal.html")
            for idx, (lat, lng, heading, movement) in tqdm(enumerate(all_positions_and_headings),
                                                           total=len(all_positions_and_headings)):
                # Store frame data with the movement from all_positions_and_headings
