...
def analyze_street_view(image_directory, question_list, out_dir):
    ...
    if os.path.exists(f"{image_directory}/frames_surround"):
...
def main():
    ...
    for map_name in map_list:
        ...
        out_dir = f"{result_dir}/multi_stop_auto_mnerged_q/{map_name}"
        ...
        GT_location_list_list = []
        correct_choice_optm_list = []

        driving_direction_ans = []
        walking_direction_ans = []
        driving_distance_ans = []
        walking_distance_ans = []
        optm_ans = []
        for i in range(len(task_list)):
            ...
            ans_choice_dict = {"direction_driving": {"gt": correct_idx_driving, "choices": final_choices_driving},
                               ...
                               "distance_driving": {"gt": correct_idx_distance_driving,
                                                    "choices": final_choices_distance_driving},
                               "distance_walking": {"gt": correct_idx_distance_walking,
                                                    "choices": final_choices_distance_walking},
            ...
            GT_location_list_list.append(GT_location_list)
            correct_choice_optm_list.append(final_choices_optm[correct_idx_optm])

            walking_direction_ans.append(correct_idx_walking)
            driving_direction_ans.append(correct_idx_driving)
            walking_distance_ans.append(correct_idx_distance_walking)
            driving_distance_ans.append(correct_idx_distance_driving)
            optm_ans.append(correct_idx_optm)
            ...
            question_list += [
                ...
                f"Put your single letter choice answer within a [[[letter]]] triple square bracket. "
                f"Do not put anything else in triple square bracket.\n"
        ...
        if results:
            ...
            for question, answer in results.items():
                ...
                # print(f"{answer}")
                set_idx = idx // 5
                set_out_dir = f"{out_dir}/set_{set_idx}"

                with open(f"{out_dir}/Q&A.md", "a", encoding="utf-8") as file:
                    file.write(f"{question}")
                    file.write(f"{answer}")
                    file.write(f"\n----------\nTrue Correct Answer: "
                               f"{idx2letter[[walking_direction_ans[set_idx], driving_direction_ans[set_idx], walking_distance_ans[set_idx], driving_distance_ans[set_idx], optm_ans[set_idx]][idx]]}\n==========\n")
                ...
                route_data_walk = {
                    "map": map_name,
                    "ans": idx2letter[walking_direction_ans[set_idx]],
                    "frames": [],
                }
                all_positions_and_headings, _, _ = get_routes_from_locations(GT_location_list_list[set_idx],
                                                                             route_file_output=f"{set_out_dir}/route_map_multi_stop_walikng.html",
                                                                             route_mode="walking")
                for j, (lat, lng, heading, movement) in tqdm(enumerate(all_positions_and_headings),
                ...
                route_data_drive = {
                    "map": map_name,
                    "ans": idx2letter[driving_direction_ans[set_idx]],
                    "frames": [],
                }
                all_positions_and_headings, _, _ = get_routes_from_locations(GT_location_list_list[set_idx],
                ...
                route_data_optimal = {
                    "map": map_name,
                    "ans": idx2letter[optm_ans[set_idx]],
                    "frames": [],
                }
                all_positions_and_headings, _, _ = get_routes_from_locations(correct_choice_optm_list[set_idx], f"{set_out_dir}/route_map_multi_stop_optimal.html")
                for j, (lat, lng, heading, movement) in tqdm(enumerate(all_positions_and_headings),
                                                               total=len(all_positions_and_headings)):
                    # Store frame data with the movement from all_positions_and_headings
